...
{
    public class ClientService : CommonService<Client, IReportItem>
    {
        ...
        public override async Task<Client?> Save(Client? model)
        {
            if (model == null)
            {
            ...
            _httpClient.DefaultRequestHeaders.Accept.Clear();
            _httpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
            var getTask = _httpClient.PostAsJsonAsync("Clients/Save", model);
